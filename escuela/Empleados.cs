...
namespace escuela
{
	...
	public Hashtable obtenerPorId(string id){
			...
			this.abrirConexion();
            MySqlCommand myCommand = new MySqlCommand("SELECT * FROM empleados WHERE id = '"+id+"'", 
			                                          myConnection);
            MySqlDataReader myReader = myCommand.ExecuteReader();	
            if(myReader.HasRows){
	        	myReader.Read();
		...
		public void insertarRegistroNuevo(string nombre,string domicilio,string telefono){
			this.abrirConexion();
string sql = "INSERT INTO `empleados` (`nombre`,`domicilio`,`telefono`) VALUES ('" + nombre + "','" + domicilio + "','" + telefono +"')";
		...
		public void editar(string id,string nombre,string domicilio, string telefono){
			this.abrirConexion();
			string sql = "UPDATE `empleados` SET `id`='" + id + "', `nombre`='" + nombre + "' , `domicilio`='" + domicilio + "' , `telefono`='" + telefono + 
			"' WHERE (`id`='" + id + "')";
			this.ejecutarComando(sql);
			this.cerrarConexion();
		}
